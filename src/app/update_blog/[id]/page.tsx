...
function UpdateBlog({ params: { id } }: { params: { id: string } }) {
  const router = useRouter();
  const [updatePost, updateresult] = useUpdateBlogMutation();
  const [deletePost, deleteresult] = useDeleteBlogMutation();
  const { data, isFetching, idError } = useGetBlogByIdQuery(id);
  ...
  useEffect(() => {
    ...
    const authorJSON = JSON.parse(author);

    if (data?.author_id !== authorJSON.id) router.replace("/");
  }, [data?.author_id]); //eslint-disable-line
